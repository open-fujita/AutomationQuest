...
export const M2: Mission = {
  ...
  goals: [
    '【ステップ①】ブラウザビューでセルをクリック → 下部のタグパスバーで「table」をクリックしてテーブル全体（緑枠）を選択する',
    '【ステップ②】テーブルを右クリック →「ループ → テーブル行繰り返し → 最初の行を除外」を選ぶ（ヘッダ行をスキップして各データ行を反復）',
    '【ステップ③】青枠（現在反復行）内のセルを右クリック →「抽出 → テキスト → 新しいコンプレックス タイプの変数…」でタイプ・属性・変数を作成する',
    '同じ手順で 担当者・電話 の列も抽出して 3 つの属性すべてを埋める',
    '「ページを読み込む」で取引先一覧ページ（URL を確認）を設定してから、実行して 6 件すべてが取れることを確認する',
  ],
  site: SITE,
  ...
  checks: [
    ...
    requireForEach('「テーブル行繰り返し」で各行をループする', '🖱 テーブルを右クリック →「ループ → テーブル行繰り返し → 最初の行を除外」を選んでください。ヘッダ行をスキップして全データ行を反復します。'),
    requireExtractCount(3, '3 つの列を抽出する', '🖱 青枠（現在反復行）内のセルを右クリック →「抽出 → テキスト → 新しいコンプレックス タイプの変数…」で 3 列（会社名・担当者・電話）を抽出してください。'),
