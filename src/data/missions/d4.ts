...
export const D4: Mission = {
  ...
  dasChecks: [
    requireForEachScope(
      'For Each にスコープファインダーが設定されている',
      'For Each ステップのスコープファインダーに仕入れ一覧テーブルのセレクタ（例: `table[name="仕入れ一覧"]`）を設定してください。スコープがないと全画面から要素を探してしまいます。',
    ),
    requireRelativeSelector(
      'エレメントファインダーに相対セレクタ（「> ...」形式）を設定している',
      'For Each のエレメントファインダーに相対セレクタ（例: `> listitem`）を設定してください。「>」はスコープの直接の子を表します。',
    ),
    requireDasExtractCount(
      '品目',
      3,
