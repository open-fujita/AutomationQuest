...
export const D4: Mission = {
  ...
  goals: [
    '「For Each（要素の繰り返し）」ステップを追加する',
    'スコープファインダーに仕入れ一覧テーブル（`table[name="仕入れ一覧"]`）を設定する',
    'エレメントファインダーに相対セレクタ（`> listitem`）を設定する',
  ...
  dasChecks: [
    requireForEachScope(
      ...
      '仕入れ一覧テーブル（`table[name="仕入れ一覧"]`）を右クリックして For Each を作成し、実行してください。タイトルラベルやボタンなど一覧以外の要素にループを作っても、スコープが解決できないためクリアになりません。',
    ),
    requireRelativeSelector(
      'エレメントファインダーに相対セレクタ（「> ...」形式）を設定し、実行で 2 件以上を反復できた',
      'For Each のエレメントファインダーに相対セレクタ（例: `> listitem`）を設定し、実行してください。一覧コンテナ以外にループを作った場合は反復件数が足りずクリアになりません。実行して仕入れ一覧の全行が処理されることを確認してください。',
    ),
    requireDasExtractCount(
      '品目',
      3,
      '品目変数に 3 件以上の抽出結果がある',
      'For Each の body 内で「値を抽出」を追加し、品目名を「品目」変数に格納してください。実行して 3 件以上取れているか確認してください。',
    ),
    requireDasNoErrors(
      '実行時にエラーが無い',
