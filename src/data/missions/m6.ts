...
export const M6: Mission = {
  ...
  goals: [
    ...
    '「ページを読み込む」→「テーブル行繰り返し」で各行をループする',
    '一段目: 承認状態セルを右クリック →「値判定」で 承認状態 が「未承認」の行だけを残す',
    '二段目: 金額区分セルを右クリック →「値判定」で さらに 金額区分 が「高額」の行だけを残す',
    '件名・承認状態・金額区分のセルを「抽出」して変数に格納する',
  ...
  checks: [
    ...
    requireTestValue('未承認', '一段目の「値判定」で「未承認」だけを残す', '🖱 青枠内の承認状態セルを右クリック →「値判定」→ 条件を「等しい」「未承認」にしてください。抽出より前に置きます。'),
    requireTestValue('高額', '二段目の「値判定」で「高額」だけを残す', '🖱 金額区分セルを右クリック →「値判定」→ 条件を「等しい」「高額」にしてください。一段目の後ろ、抽出の前に置きます。'),
    requireExtractCount(2, '承認状態と金額区分の 2 列を抽出する', '🖱 青枠内のセルを右クリック →「抽出 → テキスト」で承認状態と金額区分を変数に格納してください。値判定の後ろに置きます。'),
