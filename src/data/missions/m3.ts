...
export const M3: Mission = {
  ...
  goals: [
    ...
    '「ページを読み込む」→「要素の繰り返し」で各行をループする',
    '状態セルを右クリック →「値判定」で、状態 が「未対応」と等しい行だけを残す（条件に合わない行はスキップ）',
    '件名・状態・担当のセルを「抽出」して変数に格納する',
  ...
  checks: [
    ...
    requireTestValue('未対応', '「値判定」で状態が「未対応」の行だけを残す', '🖱 青枠内の状態セルを右クリック →「値判定」→ 条件を「等しい」「未対応」にしてください。ループの中で抽出より前に置きます。'),
    requireExtractCount(2, '少なくとも 2 つの列（状態を含む）を抽出する', '🖱 青枠（現在反復行）内のセルを右クリック →「抽出 → テキスト」で件名・状態・担当を変数に格納してください。'),
    requireMaxRecordCountEquals(MIKAITOU, '未対応の 4 件だけに絞り込めている', '⚙ 値判定の条件を見直してください。全件（7 件）のままなら値判定が効いていません。状態セルに対して「未対応」と等しい、になっているか確認を。'),
