  ...
  return (
    <div className="flex min-h-0 flex-col">
      ...
      <div className="min-h-0 flex-1 overflow-auto p-2 text-[12px]">
        <div className="text-ds-text">📁 自動化推進室</div>
        <div className="ml-3 mt-0.5">
          <div className="flex items-center gap-1 rounded bg-ds-accent2/15 px-1.5 py-0.5 text-ds-text">
            🤖 {robot.name}.robot
          </div>
          <div className="ml-3 mt-1 text-ds-textDim">
            <div className="mb-0.5">📂 タイプ</div>
            {robot.types.length === 0 && <div className="ml-3 text-[11px] text-ds-textDim/70">（なし）</div>}
            {robot.types.map((t) => (
            ...
            <div className="ml-3 text-[11px] text-ds-textDim/70">（M6 で登場）</div>
          </div>
        </div>
      </div>
    </div>
